...
pub fn call(name: &str, args: &[RtValue]) -> Result<RtValue, String> {
    match name {
        ...
        "round" => {
            ...
            let unit = if args.len() >= 2 { arg_num(args, 1)? } else { 1.0 };
            if unit == 0.0 { return Ok(RtValue::Num(x)); }
            Ok(RtValue::Num((x / unit).round() * unit))
        }
        "mod" => {
            let a = arg_num(args, 0)?;
            let b = arg_num(args, 1)?;
            if b == 0.0 { return Ok(RtValue::missing()); }
        ...
        "substr" => {
            ...
            if start < 1 { return Ok(RtValue::Str(String::new())); }
            let start_idx = (start - 1) as usize;
            if start_idx >= chars.len() { return Ok(RtValue::Str(String::new())); }
        ...
        "cats" => Ok(RtValue::Str(args.iter().map(|v| v.as_str().trim().to_string()).collect::<Vec<_>>().join(""))),
        "catx" => {
            if args.is_empty() { return Ok(RtValue::Str(String::new())); }
            let sep = args[0].as_str().to_string();
            let parts: Vec<String> = args[1..].iter().map(|v| v.as_str().trim().to_string()).filter(|s| !s.is_empty()).collect();
        ...
        "index" => {
            let s = arg_str(args, 0)?;
            let needle = arg_str(args, 1)?;
            if needle.is_empty() { return Ok(RtValue::Num(0.0)); }
        ...
        "coalesce" => {
            for v in args {
                if !is_missing(v) { return Ok(v.clone()); }
            }
            Ok(RtValue::missing())
        }
        "coalescec" => {
            for v in args {
                let s = v.as_str();
                if !s.trim().is_empty() { return Ok(RtValue::Str(s.to_string())); }
            }
        ...
        "year" => Ok(date_part(args, |d| { use chrono::Datelike; d.year() as f64 })),
        "month" => Ok(date_part(args, |d| { use chrono::Datelike; d.month() as f64 })),
        "day" => Ok(date_part(args, |d| { use chrono::Datelike; d.day() as f64 })),
        "weekday" => Ok(date_part(args, |d| {
            use chrono::Datelike;
            // SAS weekday: Sunday=1 … Saturday=7
            (d.weekday().num_days_from_sunday() + 1) as f64
        })),
        "qtr" => Ok(date_part(args, |d| { use chrono::Datelike; ((d.month() - 1) / 3 + 1) as f64 })),
        "mdy" => {
        ...
        "hour" => Ok(RtValue::Num((arg_num(args, 0)?.rem_euclid(86400.0) / 3600.0).floor())),
        "minute" => Ok(RtValue::Num(((arg_num(args, 0)?.rem_euclid(86400.0)) % 3600.0 / 60.0).floor())),
        ...
        other => Err(format!("function '{}' is not implemented in PAS v0.5", other)),
    }
}
...
fn parse_format_spec(spec: &str) -> Result<FormatSpec, String> {
    ...
    let (width, decimals) = if suffix.is_empty() {
        ...
        let w = if dot == 0 { None } else { suffix[..dot].parse().ok() };
        let d = suffix[dot + 1..].parse().ok();
        (w, d)
    } else {
        (suffix.parse().ok(), None)
    };
    Ok(FormatSpec { is_char, name, width, decimals })
}
    ...
    Ok(match fmt.name.as_str() {
        ...
        "mmddyy" => match n.and_then(sas_date_to_naive) {
            Some(d) => {
                ...
                if w == 8 { format!("{:02}/{:02}/{:02}", d.month(), d.day(), d.year() % 100) }
                else { format!("{:02}/{:02}/{:04}", d.month(), d.day(), d.year()) }
            }
            None => ".".to_string(),
        },
        "ddmmyy" => match n.and_then(sas_date_to_naive) {
            Some(d) => {
                use chrono::Datelike;
                let w = fmt.width.unwrap_or(10);
                if w == 8 { format!("{:02}/{:02}/{:02}", d.day(), d.month(), d.year() % 100) }
                else { format!("{:02}/{:02}/{:04}", d.day(), d.month(), d.year()) }
            }
            None => ".".to_string(),
        },
...
fn input_value(s: &str, spec: &str) -> Result<RtValue, String> {
    ...
    let s = s.trim();
    Ok(match fmt.name.as_str() {
        "" | "best" => s.parse::<f64>().map(RtValue::Num).unwrap_or_else(|_| RtValue::missing()),
        "comma" => s.replace(',', "").trim().parse::<f64>().map(RtValue::Num).unwrap_or_else(|_| RtValue::missing()),
        "date" => super::lex::parse_sas_date(s).map(RtValue::Num).unwrap_or_else(|_| RtValue::missing()),
        "time" => super::lex::parse_sas_time(s).map(RtValue::Num).unwrap_or_else(|_| RtValue::missing()),
        "datetime" => super::lex::parse_sas_datetime(s).map(RtValue::Num).unwrap_or_else(|_| RtValue::missing()),
        "mmddyy" | "ddmmyy" | "yymmdd" => parse_slashed_date(s, &fmt.name).map(RtValue::Num).unwrap_or_else(|_| RtValue::missing()),
...
fn format_sas_date(d: chrono::NaiveDate) -> String {
    use chrono::Datelike;
    let m = ["JAN","FEB","MAR","APR","MAY","JUN","JUL","AUG","SEP","OCT","NOV","DEC"][d.month() as usize - 1];
...
fn date_part(args: &[RtValue], f: impl Fn(chrono::NaiveDate) -> f64) -> RtValue {
    match args.first().and_then(|v| v.as_num()).and_then(sas_date_to_naive) {
...
fn intnx(interval: &str, start: f64, n: i64) -> RtValue {
    ...
    let Some(d) = sas_date_to_naive(start) else { return RtValue::missing(); };
...
fn intck(interval: &str, a: f64, b: f64) -> RtValue {
    ...
    let n = match interval {
        ...
        "month" => (db.year() as i64 * 12 + db.month() as i64) - (da.year() as i64 * 12 + da.month() as i64),
...
fn reduce_num(args: &[RtValue], init: f64, f: fn(f64, f64) -> f64) -> RtValue {
    let nums: Vec<f64> = args.iter().filter_map(|v| v.as_num()).collect();
    if nums.is_empty() { RtValue::missing() }
    else { RtValue::Num(nums.into_iter().fold(init, f)) }
}
